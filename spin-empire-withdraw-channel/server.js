...
import { GAMES, ITEMS, PLINKO, PLINKO_TABLES, gameState, gameTransaction, updateCrash } from './games.js';
...
function loadDb() {
  try { return JSON.parse(fs.readFileSync(dataFile, 'utf8')); }
    ...
    return { users: {}, lottery: { pot: 3000000, entrants: {}, drawAt: Date.now() + 3 * 86400000 } };
  }
}
  ...
  try {
    ...
    for (const rain of Object.values(db.rains || {}).filter(r => r.status === 'paid' && !r.announced)) {
      try {
        ...
        await channel.send({
          content: `🌧️ Rain ended: ${Object.keys(rain.claimants).length} claimants received ${rain.share.toLocaleString()} coins each. ${rain.remainder.toLocaleString()} leftover coins carry into the next rain.`,
          allowedMentions: { parse: [] }
        });
        rainTransaction(draft => { draft.rains[rain.id].announced = true; });
      } catch {
        console.error('Rain paid, but end announcement failed; will retry.');
      }
    }
  } catch {
    console.error('Rain settlement failed; will retry. Check persistence and balance limits.');
  } finally {
    rainTickBusy = false;
  }
}
...
function takeBet(user, rawBet) {
  ...
  if (!Number.isSafeInteger((user.wagered || 0) + bet)) throw new Error('Wager total limit exceeded.');
  user.balance -= bet;
  user.wagered += bet;
  return bet;
}
...
async function guildContext(guildId, userId) {
  if (!discordClient) throw new Error('Discord bot is not connected.');
  if (!/^\d{16,22}$/.test(String(guildId || ''))) throw new Error('Launch Spin Empire from a Discord server to use this feature.');
  const guild = await discordClient.guilds.fetch(String(guildId));
  const member = await guild.members.fetch({ user: userId, force: true });
  const roles = await guild.roles.fetch();
  return { guild, member, isAdmin: hasOwnerRole([...member.roles.cache.keys()], [...roles.values()]) };
}

function activeRainForGuild(guildId, userId) {
  const rain = Object.values(db.rains || {}).find(r => r.guildId === guildId && r.status === 'open' && r.endsAt > Date.now());
  if (!rain) return null;
  return { id: rain.id, amount: rain.amount, endsAt: rain.endsAt, claimants: Object.keys(rain.claimants || {}).length, claimed: Boolean(rain.claimants?.[userId]) };
}
...
app.get('/api/games', auth, (req,res) => {
  ...
  res.json({ ...gameState(db.users[req.user.id],now), games: GAMES, items: ITEMS, plinko: PLINKO, plinkoTables: PLINKO_TABLES });
});

app.post('/api/game/:game', auth, (req,res) => {
  try {
    res.json(rainTransaction(draft => gameTransaction(draft,req.user.id,req.body.requestId,req.params.game,req.body)));
  } catch (error) {
  ...
  try {
    const response = await fetch('https://discord.com/api/oauth2/token', {
      ...
      body: new URLSearchParams({
        ...
        code: req.body.code,
        ...(req.body.redirectUri ? { redirect_uri: req.body.redirectUri } : {})
      })
    });
...
app.get('/api/community', auth, async (req,res) => {
  ...
  try {
    let isAdmin = false;
    if (guildId) {
      try { isAdmin = (await guildContext(guildId, req.user.id)).isAdmin; } catch {}
    }
    res.json({ isAdmin, rain: guildId ? activeRainForGuild(guildId, req.user.id) : null });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
});

app.post('/api/rain/claim', auth, async (req,res) => {
  const guildId = String(req.body.guildId || '');
  try {
    const { member } = await guildContext(guildId, req.user.id);
    const rain = rainService.claim(guildId, { id: req.user.id, username: req.user.username, avatar: req.user.avatar, bot: member.user.bot });
    res.json({ message: `You're in! Your share will be credited when the rain ends.`, rain: { id: rain.id, amount: rain.amount, endsAt: rain.endsAt, claimants: Object.keys(rain.claimants).length, claimed: true } });
  } catch (error) {
    res.status(400).json({ error: error.message || 'Could not claim rain.' });
  }
});

app.post('/api/admin/give', auth, async (req,res) => {
  const guildId = String(req.body.guildId || '');
  const username = String(req.body.username || '').trim();
  const amount = Math.floor(Number(req.body.amount));
  const requestId = String(req.body.requestId || '');
  try {
    const { isAdmin } = await guildContext(guildId, req.user.id);
    if (!isAdmin) throw new Error('Only members with the role named exactly "owner" can use the admin panel.');
    if (!username) throw new Error('Enter a Discord username.');
    if (!requestId || requestId.length > 100) throw new Error('Invalid admin request.');
    const matches = Object.values(db.users).filter(u => String(u.username || '').toLowerCase() === username.toLowerCase());
    if (!matches.length) throw new Error('That Discord username has not used Spin Empire yet. Have them open the Activity once first.');
    if (matches.length > 1) throw new Error('More than one saved player has that username.');
    const target = matches[0];
    validateGrant(target.balance, amount);
    db.adminGrants ||= {};
    if (db.adminGrants[requestId]) return res.json({ message: 'That grant was already applied.', target: cleanUser(target) });
    target.balance += amount;
    db.adminGrants[requestId] = { guild: guildId, actor: req.user.id, recipient: target.id, amount, at: new Date().toISOString(), source: 'activity' };
    saveDb();
    res.json({ message: `Added ${amount.toLocaleString()} virtual coins to ${target.username}.`, target: cleanUser(target) });
  } catch (error) {
    res.status(400).json({ error: error.message || 'Could not grant coins.' });
  }
});

app.get('/api/state', auth, (req, res) => res.json({
  user: cleanUser(req.user),
  lottery: { pot: db.lottery.pot, entrants: Object.keys(db.lottery.entrants).length, tickets: db.lottery.entrants[req.user.id] || 0, drawAt: db.lottery.drawAt }
}));
...
async function findWithdrawChannelInGuild(guild) {
  const channels = await guild.channels.fetch();
  return [...channels.values()].find(ch => ch?.name === 'withdraws' && ch.isTextBased() && typeof ch.send === 'function') || null;
}
...
async function getWithdrawChannel() {
  ...
  if (configuredGuildId) {
    try {
      const guild = await discordClient.guilds.fetch(configuredGuildId);
      const channel = await findWithdrawChannelInGuild(guild);
      if (channel) return channel;
      throw new Error('Create a text channel named exactly "withdraws" in your Discord server.');
    } catch (error) {
      if (error.message?.includes('Create a text channel')) throw error;
      throw new Error('Could not access the Discord server configured by DISCORD_GUILD_ID.');
    }
  }
...
app.post('/api/play/:game', auth, (req, res) => res.status(410).json({ error: 'This game endpoint was replaced. Reload the Activity.' }));
  ...
  if (process.env.DISCORD_BOT_TOKEN) {
    const client = await startAdminBot(process.env.DISCORD_BOT_TOKEN, (profile, amount, interaction) => {
      ...
      try {
        ...
        db.adminGrants[interaction.id] = { guild: interaction.guildId, actor: interaction.user.id, recipient: profile.id, amount, at: new Date().toISOString(), source: 'slash-command' };
        saveDb();
        return user.balance;
      } catch (error) {
        db = before;
    ...
    console.warn('DISCORD_BOT_TOKEN is missing: /admin give is disabled.');
  }
