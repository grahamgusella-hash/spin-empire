...
export const GAMES = [
  ...
  { id: 'chicken', name: 'Chicken Road', icon: '🐔', tag: 'STEP OR CASH OUT', rules: 'Choose Easy, Medium, or Hard. Each difficulty changes the chance of a safe crossing and the payout growth. Cash out after any safe step. A collision loses the stake. 15 steps maximum, then automatic cash-out.' },
  ...
  { id: 'upgrader', name: 'Item Upgrader', icon: '⇧', tag: 'RISK & REWARD', rules: 'Select an owned item and a higher-value target. Success chance = 0.95 × source value / target value. The source is consumed on every attempt; success awards the target, failure awards nothing. No additional coin bet.' },
  ...
  { id: 'plinko', name: 'Plinko', icon: '⠿', tag: '12 ROWS', rules: 'Choose Easy, Medium, or Hard risk. All modes use 12 independent left/right bounces, but the payout pockets become more volatile at higher difficulty. The animation shows the server-selected path.' },
  { id: 'mines', name: 'Mines', icon: '✹', tag: 'FIND THE GEMS', rules: 'Choose Easy, Medium, or Hard. Easy uses 3 mines, Medium uses 5, and Hard uses 10 on a 25-tile board. Pick safe tiles then cash out. Gross multiplier after k safe picks is 0.97 divided by the probability of surviving k picks without replacement. A mine loses the bet.' },
...
function debit(user, bet) {
  assert(Number.isSafeInteger(bet) && bet >= 100, 'Bet must be a whole number of at least 100 coins.');
  assert(user.balance >= bet, 'Not enough coins.');
  assert(Number.isSafeInteger((user.wagered || 0) + bet), 'Wager total limit exceeded.');
  user.balance -= bet;
...
export function perform(user, game, body, now = Date.now(), rng = rngDefault) {
  ...
  if (game === 'chicken' && action === 'step') { const safeChance = r.safeChance || CHICKEN_SAFE[r.difficulty || 'medium']; if (rng(10000) >= Math.floor(safeChance * 10000)) finish(user,0,'Collision! The round is over.'); else { r.steps++; r.multiplier = 0.97 / safeChance ** r.steps; r.message = `Safe crossing ${r.steps}!`; if (r.steps === 15) finish(user,r.multiplier,'Road completed — auto cashed out.'); } return; }
  if (game === 'hilo' && ['higher','lower'].includes(action)) { const favorable = action === 'higher' ? 13-r.rank : r.rank-1; assert(favorable > 0,'That direction cannot win.'); const previous = r.rank; r.rank = rng(13)+1; if (!(action === 'higher' ? r.rank > previous : r.rank < previous)) finish(user,0,r.rank === previous ? 'Tie — round lost.' : 'Wrong direction.'); else { r.steps++; r.multiplier = Math.min(1000000,r.multiplier * 0.97 / (favorable/13)); r.message = 'Correct! Keep going or cash out.'; if (r.steps === 10 || r.multiplier === 1000000) finish(user,r.multiplier,'Round limit reached — auto cashed out.'); } return; }
  if (game === 'mines' && action === 'pick') { assert(Number.isInteger(body.tile) && body.tile >= 0 && body.tile < 25 && !r.picked.includes(body.tile),'Choose an unopened tile.'); r.picked.push(body.tile); if (r.bombs.includes(body.tile)) finish(user,0,'Mine hit.'); else { let chance = 1; for (let i = 0; i < r.picked.length; i++) chance *= (25-r.mines-i)/(25-i); r.multiplier = 0.97/chance; r.message = 'Gem found!'; if (r.picked.length === 25-r.mines) finish(user,r.multiplier,'All gems found — auto cashed out.'); } return; }
...
export function visibleRound(user, now = Date.now()) {
  ...
  for (const k of ['id','version','game','status','bet','payout','multiplier','message','steps','rank','point','dice','mines','picked','result','number','color','path','bin','item','chance','success','from','target','difficulty','safeChance','pockets']) if (r[k] !== undefined) out[k] = r[k];
