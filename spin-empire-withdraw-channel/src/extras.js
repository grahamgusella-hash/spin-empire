...
const crates = {
  starter:{name:'Starter Crate',icon:'▣',price:1000,desc:'Basic drops with a chance at Diamond.'},
  miner:{name:'Miner Crate',icon:'⛏',price:5000,desc:'Better odds for Gold, Emerald and Diamond.'},
  royal:{name:'Royal Crate',icon:'♛',price:25000,desc:'Premium crate with Crown and Nether Star chances.'},
  mythic:{name:'Mythic Crate',icon:'✦',price:100000,desc:'High-tier crate with the strongest rare-item odds.'}
};
...
const tickets = {
  bronze:{name:'Bronze Scratch',price:1000,className:'bronze'},
  silver:{name:'Silver Scratch',price:10000,className:'silver'},
  gold:{name:'Gold Scratch',price:100000,className:'gold'}
};
...
function renderScratch() {
  ...
  openOverlay(`<div class="scratch-ticket ${ticket.className}"><div class="scratch-head"><div><small>SPIN EMPIRE</small><h2>${ticket.name}</h2></div><strong>${fmt(r.bet)} COINS</strong></div><p class="scratch-instructions">Scratch all nine spots. Match 3 prize symbols to win.</p><div class="scratch-grid">${symbols.map((symbol,i)=>`<button class="scratch-spot ${symbol?'revealed':''}" data-scratch="${i}" ${symbol||r.status==='done'?'disabled':''}>${symbol?`<span>${esc(symbol)}</span>`:'<span>SCRATCH</span>'}</button>`).join('')}</div><div class="scratch-status"><b>${esc(r.message || '')}</b>${r.status==='done'?`<strong class="${r.payout>0?'win':'loss'}">Payout: ${fmt(r.payout)} coins</strong>`:`<span>${r.revealed?.length||0}/9 revealed</span>`}</div></div><div class="extra-message"></div><div class="scratch-actions">${r.status==='done'?'<button id="new-ticket">Buy another ticket</button>':''}</div>`);
  overlay.querySelectorAll('[data-scratch]').forEach(button=>button.onclick=async()=>{
    if(button.disabled)return;
    button.disabled=true;
    try {
      await gameApi('scratch',{action:'reveal',index:Number(button.dataset.scratch),roundId:r.id,version:r.version});
      renderScratch();
