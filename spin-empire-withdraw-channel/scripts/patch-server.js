...
if (!source.includes("app.post('/api/login-ping'")) {
  ...
  source = source.slice(0, insertAt) + "app.post('/api/login-ping', (req,res) => { res.set('Cache-Control','no-store'); res.json({ ok:true, marker:'server-1718' }); });\n\n" + source.slice(insertAt);
} else {
  source = source.replace(/marker:'server-[^']+'/g, "marker:'server-1718'");
}
...
const tokenRoute = `app.post('/api/token', async (req, res) => {
  ...
  try {
    ...
    let launch = null;
    for (let i = 0; i < 30 && !launch; i += 1) {
      launch = consumeActivityLaunch(instanceId);
      if (!launch) await new Promise(resolve => setTimeout(resolve, 100));
    }
    if (!launch) {
      return res.status(401).json({ error: 'Spin Empire could not verify this /casino launch. Close it and run /casino again.' });
    }
    ...
    return res.status(500).json({ error: 'Spin Empire login failed. Close it and run /casino again.' });
  }
});
...
if (!source.includes("marker:'server-1718'")) throw new Error('Server marker update was not applied.');
...
console.log('Patched server to authenticate /casino launches by Discord Activity instance (server-1718).');
