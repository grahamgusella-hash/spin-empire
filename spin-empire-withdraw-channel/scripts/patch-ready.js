...
const pingRaw = await pingResponse.text();
let pingData;
try { pingData = pingRaw ? JSON.parse(pingRaw) : {}; }
catch { throw new Error(\`Server probe returned HTTP \${pingResponse.status}, not JSON: \${pingRaw.replace(/\\s+/g,' ').trim().slice(0,120) || '(empty response)'}\`); }
...
if (!source.includes("fetch('/api/login-ping'")) throw new Error('Server reachability probe was not applied.');
...
console.log(`Patched startup with server reachability probe (${BUILD_MARKER}).`);
